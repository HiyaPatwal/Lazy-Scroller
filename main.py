...
USE_SMALL_LOW_FATIGUE_BOX = True
...
def main():
    ...
    print("Lazy Scroller Active. Press Q to quit.")
    ...
    while True:
        ...
        if len(landmark_list) == 0:
            cv2.putText(display_frame, "STATUS: NO HAND DETECTED",
                        (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                        (0, 0, 255), 2)
        else:
            fingers = tracker.fingers_up(landmark_list)

            cv2.putText(display_frame, f"GEOFENCE: HAND ACTIVE {fingers}",
                        (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                        (0, 255, 0), 2)
            ...
            # ---------- SCROLL ----------
                ...
                scroll_amount = int((center_y - y_index) * SCROLL_MULTIPLIER)

                if abs(scroll_amount) > 5:
                    pyautogui.scroll(scroll_amount)

        cv2.imshow("Lazy Scroller", display_frame)
